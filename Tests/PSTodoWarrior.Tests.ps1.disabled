<#
    .Synopsis
        Tests each script file in the project against a standard set of tests.
    .Description
        Tests each script file in this project against a standard set of tests:

        * PowerShell errors;
        * PSScriptAnalyzer;
        * Has an associated Pester test file;

        Once these standard tests are run the individual Pester test file is also run.

        ** This script expects to live in the Tests folder in the root of the project folder. **
#>

# in this array put the script analyzer rules that you don't want to use
Write-Host 'Creating configuration variables.' -ForegroundColor Cyan
$ExcludedRules = @("PSUseShouldProcessForStateChangingFunctions")
$ScriptSubfolder = @('Public', 'Private')
$projRoot = Split-Path -Parent (Split-Path -Path $MyInvocation.MyCommand.Path -Parent)
Write-Host "`tProject Root: $projRoot" -ForegroundColor Cyan
Write-Host "`tScript Subfolder(s): $($ScriptSubfolder -join ', ')" -ForegroundColor Cyan
#Write-Host "`tTest subfolder(s): $($TestSubfolder -join ', ')" -ForegroundColor Cyan

# loop through each folder in $ScriptSubfolder and grab any script files (ignore any Pester test filles)
$scriptFile = $ScriptSubfolder | ForEach-Object {
    $folder = Join-Path -Path $projRoot -ChildPath $_
    Get-ChildItem -Path (Join-Path -Path $folder -ChildPath "*.ps1") -Recurse | Where-Object {$_.Name -NotLike "*.Tests.ps1"}
}
Write-Host "Script files found: $($scriptFile.count)" -ForegroundColor Cyan
<#
# loop through each folder in $TestSubfolder and grab the Pester tests
$testFile = $TestSubfolder | Where-Object {
    $folder = Join-Path -Path $projRoot -ChildPath $_
    Get-ChildItem -Name (Join-Path -Path $folder -ChildPath '*.Tests.ps1') -Recurse | Where-Object { $_.Name -ne $MyInvocation.MyCommand.Name }
}
Write-Host "Test files found: $($testFile.count)" -ForegroundColor Cyan
#>
# show some pretty info about the PSScriptAnalayzer rules we're using (and excluding)
$rules = Get-ScriptAnalyzerRule | Where-Object { $_.Rulename -notin $ExcludedRules }
Write-Host "PSSCriptAnalyzer Rules: $($rules.count) rules will be used" -NoNewline -ForegroundColor Cyan
if ($ExcludedRules.count -gt 0) {
    Write-Host " and $($ExcludedRules.count) excluded. Excluded rules: $($ExcludedRules -join ", ")." -ForegroundColor Cyan
}
else {
...
<#
$manifest = Get-Item -Path (Join-Path -Path $projRoot -ChildPath "*.psd1")
Write-Host "Found module manifest file $manifest" -ForegroundColor Cyan
...
Import-Module -Name (Join-Path -Path $projRoot -ChildPath "$($module).psd1")
#>
#region Running Standard Testing
if ($scriptFile.count -gt 0) {
    foreach($script in $scriptFile)
    {
        Describe "Standard Tests on script file $($script.FullName.Replace($projRoot, ''))" {
            It "Passes if there are no Powershell errors" {
            ...
            It "Passes all Script Analyzer tests ($($ExcludedRules.count) excluded)" {
                (Invoke-ScriptAnalyzer -Path $script.FullName -ExcludeRule $ExcludedRules ).Count | Should Be 0
            }

            It 'Has a Pester test file' {
                "$($script.BaseName).Tests" -in $scriptFileTest.BaseName | Should Be $true
            }
        }
    }
}
